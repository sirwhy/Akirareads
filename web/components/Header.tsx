...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const d = await api.getSeries({ q: searchQuery, search: searchQuery, limit: 5 });
        setSearchResults(d.data || d.series || []);
  ...
  // Reader + admin render their own chrome (ported from old App.js)
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-dark-900/95 backdrop-blur-md border-b border-white/5 shadow-xl shadow-black/50' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
        ...
        <nav className="hidden md:flex items-center gap-1">
          {navLinks.map(l => (
            <Link key={l.to} href={l.to}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${pathname === l.to ? 'text-accent bg-accent/10' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>
              {l.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2">
              ...
              <div className="absolute right-0 top-12 w-72 bg-dark-700 border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-slide-up">
                <div className="p-3 border-b border-white/5">
                  <input autoFocus value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                    ...
                    onKeyDown={e => { if (e.key === 'Enter' && searchQuery) { router.push(`/browse?q=${encodeURIComponent(searchQuery)}`); setSearchOpen(false); setSearchQuery(''); } }} />
                </div>
                {searchResults.length > 0 && (
                  <div className="max-h-64 overflow-y-auto">
                    {searchResults.map(s => (
                      <Link key={s.id} href={`/series/${s.slug}`} onClick={() => { setSearchOpen(false); setSearchQuery(''); }}
                        className="flex items-center gap-3 p-3 hover:bg-white/5 transition-colors">
                        {s.cover
                          ? <img src={s.cover} alt={s.title} referrerPolicy="no-referrer" className="w-10 h-14 object-cover rounded" />
          ...
          {user ? (
            <div ref={dropdownRef} className="relative">
              ...
              {dropdownOpen && (
                <div className="absolute right-0 top-12 w-48 bg-dark-700 border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-slide-up">
                  ...
                  <div className="p-1">
                    <Link href="/profile" className="flex items-center gap-2 px-3 py-2 text-sm text-white/70 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
                      <FiUser className="text-xs" /> Profil Saya
                    </Link>
                    <Link href="/profile?tab=bookmarks" className="flex items-center gap-2 px-3 py-2 text-sm text-white/70 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
                      <FiBookmark className="text-xs" /> Bookmark
                    </Link>
                    {isAdmin && (
                      <Link href="/admin" className="flex items-center gap-2 px-3 py-2 text-sm text-accent hover:bg-accent/10 rounded-lg transition-colors">
            ...
            <div className="flex items-center gap-2">
              <Link href="/login" className="text-sm text-white/70 hover:text-white px-3 py-2 transition-colors">Masuk</Link>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-dark-800 border-b border-white/5 px-4 pb-4 animate-slide-up">
          {navLinks.map(l => (
            <Link key={l.to} href={l.to} className={`block py-3 text-sm font-medium border-b border-white/5 ${pathname === l.to ? 'text-accent' : 'text-white/60'}`}>{l.label}</Link>
          ))}
        </div>
      )}
    </header>
