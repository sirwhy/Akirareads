...
import { statusBadgeClass, statusBadgeLabel } from './types';

export default function SeriesCard({ series }: { series: SeriesSummary }) {
  return (
    <Link href={`/series/${series.slug}`} className="series-card card group block">
      <div className="relative aspect-[3/4] overflow-hidden">
        {series.cover ? (
          <img src={series.cover} alt={series.title} referrerPolicy="no-referrer" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
        ) : (
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1">
          <span className={`badge text-[10px] ${statusBadgeClass(series.status)}`}>
            {statusBadgeLabel(series.status)}
          </span>
