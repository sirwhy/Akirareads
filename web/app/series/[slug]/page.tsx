...
import type { SeriesDetail, ChapterSummary, Comment } from '@/components/types';
import { STATUS_COLOR, STATUS_LABEL, timeAgo } from '@/components/types';
...
export default function SeriesPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const data: SeriesDetail = await api.getSeriesBySlug(slug);
        setSeries(data);
        const comData = await api.getSeriesComments(data.id);
        setComments(comData || []);
        if (isLoggedIn) {
          const bookmarks = await api.getMyBookmarks();
          setBookmarked((bookmarks || []).some((b: { seriesId: string }) => b.seriesId === data.id));
        }
  ...
  const handleComment = async () => {
    ...
    if (!commentText.trim() || !series) return;
    setSubmitting(true);
    try {
  ...
  const chapters: ChapterSummary[] = [...(series.chapters || [])].sort((a, b) => b.chapterNum - a.chapterNum);
  ...
  const longDesc = (series.description || '').length > 280;
    ...
    <div className="max-w-5xl mx-auto px-4 pt-24 pb-12 animate-fade-in">
      ...
      <div className="relative rounded-2xl overflow-hidden mb-8 border border-white/5">
        ...
        <div className="absolute inset-0">
          {series.cover && <img src={series.cover} alt="" referrerPolicy="no-referrer" className="w-full h-full object-cover scale-110 blur-2xl opacity-20" />}
        ...
        <div className="relative z-10 p-6 md:p-8 flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-shrink-0 w-40 md:w-48 self-start">
            <div className="rounded-xl overflow-hidden border border-white/10 shadow-2xl aspect-[3/4]">
              {series.cover ? <img src={series.cover} alt={series.title} referrerPolicy="no-referrer" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-dark-600 flex items-center justify-center"><span className="text-4xl opacity-30">📖</span></div>}
            </div>
          </div>
          {/* Info */}
          <div className="flex-1">
            <div className="flex flex-wrap gap-2 mb-3">
              <span className={`badge text-xs ${STATUS_COLOR[series.status || ''] || 'text-white/40 bg-white/5'}`}>{STATUS_LABEL[series.status || ''] || series.status}</span>
            ...
            <div className="flex items-center gap-4 text-sm text-white/40 mb-4">
              ...
              {!!series.rating && series.rating > 0 && <span className="flex items-center gap-1 text-yellow-400"><FiStar /> {series.rating.toFixed(1)}</span>}
            </div>
            {series.genres && series.genres.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-5">
                {series.genres.map(g => <Link key={g} href={`/browse?genre=${g}`} className="badge text-xs text-white/40 bg-white/5 hover:bg-accent/10 hover:text-accent transition-colors">{g}</Link>)}
              </div>
            )}
            <p className={`text-white/60 text-sm leading-relaxed mb-6 max-w-2xl ${!descExpanded && longDesc ? 'line-clamp-4' : ''}`}>
              {series.description || 'Tidak ada deskripsi.'}
            </p>
            {longDesc && (
              <button onClick={() => setDescExpanded(!descExpanded)} className="text-accent text-xs hover:underline mb-6 flex items-center gap-1">
                {descExpanded ? <><FiChevronUp /> Sembunyikan</> : <><FiChevronDown /> Baca selengkapnya</>}
              </button>
            )}
            <div className="flex flex-wrap gap-3">
              {firstChapter && (
                <Link href={`/read/${firstChapter.id}`} className="btn-primary flex items-center gap-2">Baca dari awal</Link>
              )}
              {latestChapter && (
                <Link href={`/read/${latestChapter.id}`} className="btn-ghost flex items-center gap-2">Chapter Terbaru</Link>
              )}
              ...
              {displayChapters.map(ch => (
                <Link key={ch.id} href={`/read/${ch.id}`}
                  ...
                  <span className="text-xs text-white/30 flex items-center gap-3">
                    {!!ch.views && <span className="flex items-center gap-1"><FiEye className="text-[10px]" /> {ch.views > 1000 ? `${(ch.views / 1000).toFixed(1)}k` : ch.views}</span>}
        ...
        <div className="p-5 border-b border-white/5">
          {isLoggedIn ? (
            ...
            <p className="text-white/40 text-sm text-center">
              <Link href="/login" className="text-accent hover:underline">Login</Link> untuk menulis komentar.
            </p>
          )}
        </div>
        {/* Comments list */}
