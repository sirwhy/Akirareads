...
function BrowseInner() {
  ...
  const filters = {
    q: params.get('q') || '',
    genre: params.get('genre') || '', // comma-separated for multi-select
    status: params.get('status') || '',
    type: params.get('type') || '',
    sort: params.get('sort') || 'updatedAt',
    order: params.get('order') || 'desc',
  };
  const genreList = filters.genre ? filters.genre.split(',').filter(Boolean) : [];

  // Update URL params (resets page) — mirrors old setFilter via useSearchParams
  const setFilter = (key: string, val: string) => {
    const p = new URLSearchParams(params.toString());
    if (val) p.set(key, val); else p.delete(key);
    p.delete('page');
    router.push(`/browse${p.toString() ? `?${p.toString()}` : ''}`);
    setPage(1);
  };

  const toggleGenre = (g: string) => {
    const next = genreList.includes(g) ? genreList.filter(x => x !== g) : [...genreList, g];
    setFilter('genre', next.join(','));
  };

  const clearFilter = (key: string) => setFilter(key, '');
  const clearAll = () => { router.push('/browse'); setSearch(''); setPage(1); };

  const fetchSeries = useCallback(async () => {
    setLoading(true);
    try {
      // Send both q and search (API accepts either); server filters by first genre,
      // remaining genres intersected client-side for multi-select.
      ...
      const list: SeriesSummary[] = res.data || res.series || [];
      setSeries(list.filter(s => genreList.every(g => (s.genres || []).includes(g))));
    ...
    <div className="max-w-7xl mx-auto px-4 pt-24 pb-12 animate-fade-in">
              ...
              <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto">
                {GENRES.map(g => (
                  <button key={g} onClick={() => toggleGenre(g)}
                    className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${genreList.includes(g) ? 'border-accent text-accent bg-accent/10' : 'border-white/10 text-white/40 hover:border-white/30'}`}>{g}</button>
                ))}
              </div>
            </div>
            {/* Type + Status */}
      ...
      {activeFilters.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {genreList.length > 0
            ? genreList.map(g => (
              <span key={`g-${g}`} className="flex items-center gap-1.5 text-xs bg-accent/10 text-accent border border-accent/20 px-2.5 py-1 rounded-full">
                {g} <button onClick={() => toggleGenre(g)}><FiX className="text-[10px]" /></button>
              </span>
            ))
            : activeFilters.filter(([k]) => k !== 'genre').map(([key, val]) => (
          ...
          {genreList.length > 0 && activeFilters.filter(([k]) => k !== 'genre').map(([key, val]) => (
            <span key={key} className="flex items-center gap-1.5 text-xs bg-accent/10 text-accent border border-accent/20 px-2.5 py-1 rounded-full">
              {val} <button onClick={() => clearFilter(key)}><FiX className="text-[10px]" /></button>
            </span>
          ))}
