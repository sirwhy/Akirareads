...
import type { SeriesSummary } from '@/components/types';
...
function HeroBanner({ series }: { series: SeriesSummary[] }) {
  ...
  return (
    <div className="relative h-[520px] md:h-[600px] overflow-hidden">
      ...
      <div className="absolute inset-0">
        {s.cover ? (
          <img src={s.cover} alt={s.title} referrerPolicy="no-referrer" className="w-full h-full object-cover scale-110 blur-sm opacity-30 transition-all duration-1000" />
        ) : (
      ...
      <div className="relative z-10 h-full max-w-7xl mx-auto px-4 flex items-center">
        <div className="flex gap-8 items-center">
          ...
          <div className="hidden sm:block flex-shrink-0 w-44 h-64 rounded-xl overflow-hidden shadow-2xl border border-white/10 glow-red">
            {s.cover ? <img src={s.cover} alt={s.title} referrerPolicy="no-referrer" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-dark-600 flex items-center justify-center"><span className="text-3xl">📖</span></div>}
          </div>
          {/* Info */}
          <div className="flex-1 max-w-lg animate-fade-in" key={active}>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {(s.genres || []).slice(0, 4).map(g => (
                <Link key={g} href={`/browse?genre=${g}`} className="badge text-xs text-white/50 bg-white/5 hover:bg-accent/10 hover:text-accent transition-colors">{g}</Link>
              ))}
            </div>
            <Link href={`/series/${s.slug}`} className="btn-primary inline-flex items-center gap-2">
  ...
  return (
    <section className="mb-14">
      <div className="flex items-center justify-between mb-6">
        ...
        {link && <Link href={link} className="text-sm text-accent hover:text-accent-light flex items-center gap-1 transition-colors">{linkLabel} <FiArrowRight /></Link>}
      </div>
      {children}
    </section>
  );
...
export default function Home() {
  const [featured, setFeatured] = useState<SeriesSummary[]>([]);
  const [latest, setLatest] = useState<SeriesSummary[]>([]);
  const [popular, setPopular] = useState<SeriesSummary[]>([]);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setFeatured(featuredRes.data || featuredRes.series || []);
        setLatest(latestRes.data || latestRes.series || []);
        setPopular(popularRes.data || popularRes.series || []);
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-12">
        ...
        <Section title="Update Terbaru" icon={FiClock} link="/latest">
          {loading ? (
            ...
            <div className="text-center py-16 text-white/30">
              ...
              <Link href="/admin/login" className="inline-block mt-3 text-accent text-sm hover:underline">Pergi ke Admin →</Link>
            </div>
          )}
        </Section>
        ...
        {/* Genre chips */}
        <Section title="Jelajahi Genre" icon={FiStar} link="/browse">
          <div className="flex flex-wrap gap-2">
            {GENRES.map(g => (
              <Link key={g} href={`/browse?genre=${g}`} className="text-sm px-4 py-2 rounded-full border border-white/10 text-white/50 hover:border-accent hover:text-accent hover:bg-accent/10 transition-colors">
                {g}
              </Link>
            ))}
          </div>
