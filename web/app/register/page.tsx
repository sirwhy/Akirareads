...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Register returns { token, user }; store the token then let AuthProvider fetch the user.
      setToken(res.token);
      await refresh();
      ...
      <div className="w-full max-w-md animate-slide-up">
        <div className="bg-dark-700/50 border border-white/5 rounded-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {(form.password || form.confirm) && (
              <div className="space-y-1.5 p-3 bg-dark-600/50 rounded-xl">
                {([[checks.length, 'Minimal 6 karakter'], [checks.match, 'Password cocok']] as const).map(([ok, label]) => (
          ...
          <div className="mt-6 text-center text-sm text-white/40">
            Sudah punya akun?{' '}
            <Link href="/login" className="text-accent hover:text-accent-light font-medium transition-colors">Masuk</Link>
          </div>
        </div>
      </div>
    </div>
