...
import { timeAgo } from '@/components/types';
        ...
        {loading ? (
          ...
          <Link key={s.id} href={`/series/${s.slug}`}
            ...
            <div className="flex-shrink-0 w-14 h-20 rounded-lg overflow-hidden border border-white/5">
              {s.cover ? <img src={s.cover} alt={s.title} referrerPolicy="no-referrer" className="w-full h-full object-cover group-hover:scale-105 transition-transform" /> : <div className="w-full h-full bg-dark-600 flex items-center justify-center"><span className="text-lg opacity-30">📖</span></div>}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-4 mt-2 text-xs text-white/30">
                ...
                {s.updatedAt && <span className="flex items-center gap-1"><FiClock /> {timeAgo(s.updatedAt)}</span>}
              </div>
            </div>
          </Link>
        ))}
